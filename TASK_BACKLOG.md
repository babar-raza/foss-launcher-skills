...
> **Hardened taskcard register**: the full execution-grade breakdown of
> every open item below (required work, verification, evidence, stop
> conditions, allowed/forbidden actions, dependencies, closeout rules) lives
> in `plans/plan-hardening-addendum-from-latest-audit.md`. The table below
> stays the quick-reference summary; it is not superseded or rewritten.
...
| SYNC-3 | Governance-audit cluster: `plan-health-watchdog`, `blind-spot-audit`, `forensic-heal-sprint`, `regression-classification`, `triage-verdict-gate` | **Corrected three times.** (1)+(2) 2026-08-29: removed `plan-normalize` (already ported S-96); ported `workflow-harden` (S-121), self-contained, did not need the concurrency stack. (3) 2026-08-30, after `plan-health-watchdog` (S-122) itself turned out portable (its assumed-missing dependency, `taskcard_store.py`, already existed from SYNC-1) -- re-verified the other 4 against actual target repo state instead of trusting the un-checked "all need MISSION_AUTHORITY.yaml/refresh-run/content_eval, none of which exist here" claim. **That blanket claim was itself wrong for 2 of the 4**, discovered by reading target's actual tree, not assuming: target's `scripts/pipeline/content_eval/` (evaluate+diff CLI, 40+ evaluators) already exists -- `regression-classification`'s core need (compare current findings against a prior baseline) has a real primitive to build on, just not source's letter-grade shape (target's is PASS/WARN/FAIL findings-based); target's `scripts/pipeline/lib/triage_confirm.py` + `reconcile_triage.py` + `knowledge/{family}/{platform}/merged/` all already exist too -- but target's `triage_confirm.py` is a stale 104-line pre-fix snapshot (source's current version is 612 lines and depends on a `knowledge_delta.json` structured-delta artifact target's own `knowledge-diff` skill does not produce, only a coarser SHA/impact report) -- genuinely needs a scoping/design pass (a delta-producer prerequisite), not a trivial CLI-wrapper add as first assumed mid-investigation. `blind-spot-audit` and `forensic-heal-sprint` **are** confirmed genuinely blocked on real missing infrastructure: a full run-bound state machine (`blind_spot_audit_run.py`, `MISSION_AUTHORITY.yaml`-shaped run manifest, `ACTIVE.json` single-owner locking) plus 5+ other skill dependencies (S-34, S-90, S-87, S-45, S-67) that do not exist here and forensic-heal-sprint (S-123) additionally consumes blind-spot-audit's own taskcard output as its primary input -- confirmed by reading both source contracts in full, not assumed | 1 of 5 CLOSED (`plan-health-watchdog`, S-122). 2 of 5 (`regression-classification`, `triage-verdict-gate`) reclassified from `blocked-on-missing-infra` to `needs-a-real-scoping-pass` -- their dependencies exist but need adaptation design, tracked as their own follow-on scoping items, not attempted this pass (already large scope this session). 2 of 5 (`blind-spot-audit`, `forensic-heal-sprint`) remain genuinely blocked, confirmed not assumed |
| SYNC-4 | Content-maintenance cluster: `body-refresh`, `frontmatter-body-consistency`, `semantic-diff`, `mechanism-sweep` | **Corrected three times.** (1) 2026-08-29: originally listed `change-sweep`, `discovery-triage`, `repo-patrol` too -- already exist in this repo (ported 2026-04-27, S-102/103/104), removed. (2) 2026-08-29: read all 4 remaining skills' full source contracts: **none are lean-portable as originally assumed.** All 4 are deeply embedded in source's own knowledge-generation pipeline internals -- `body-refresh` reads `body_impact.json`/`semantic_changes.json` and writes through source's `safe_content_write()`/S-73 context under its refresh-run state machine; `semantic-diff` converts source's specific `knowledge_delta.json` shape; `mechanism-sweep` is an agent-executed workflow over source's `data/products.json`/`reports/phase_state.json` multi-product-launch tracking (this repo is a skills library FOR other repos, not itself a multi-product launcher, so the underlying use case may not even apply); `frontmatter-body-consistency` checks source's specific evidence-frontmatter schema. (3) 2026-08-30: **the "batch-reference precedent" citation used to back this was itself wrong and is retracted.** Re-verified against `SYNC_REPORT.md` (the actual 2026-04-17 sync report, not assumed from memory): `batch-reference` (S-62) was listed there as "deferred (template-based batch gen)", not permanently rejected -- and was in fact successfully ported just 4 days later, 2026-04-21 (commit `2b1d04c`), and is live today as target's own `batch-reference` (S-67, see `docs/id-mapping.md`). The prior citation, first written this session in commit `7b82574`, was never actually checked against `SYNC_REPORT.md` before being used -- caught only on reverification per this doc's own Anti-Overclaim rule 4 (`plans/plan-hardening-addendum-from-latest-audit.md` §11). The underlying technical finding (deep coupling to source's knowledge-pipeline internals, no target-native abstraction) is independently verified from reading the actual source contracts and stands on its own -- it does not depend on this retracted citation | Do not port these as-is. Before attempting any of them, design the missing generic abstraction first (a generic "structured content-diff -> regeneration strategy" pattern) -- or explicitly mark `OUT_OF_SCOPE` with the corrected reasoning above (no longer citing a false precedent) |
| SYNC-5 | ✅ SCOPED 2026-08-30 (not ported): the "Java/Maven cluster" label was itself wrong. Read all 5 source contracts in full: only `foss-java-publish` (S-110, 1176 lines) is actually Java/Maven-specific (pom.xml/GPG/Sonatype Central Portal + deep Aspose 5-subdomain content-healing integration -- the single most tightly-coupled skill read this whole sync). `media-optimize` (S-119) has **zero** Java/Maven connection -- it's generic Hugo static-asset optimization (WebP+PNG dual-format generation, byte-budget compliance, quality-ladder tuning); genuinely portable with medium effort (needs Pillow+oxipng as new deps, and its `static/media/{family}/{platform}/` path convention generalized to config-driven, same pattern already used for `content_path`) -- **not attempted this pass**, filed as its own item below, no longer miscategorized as Java/Maven. `package-publish-watch` (S-118) explicitly excludes Maven/Java by its own stated scope ("Do NOT use S-118 for: Maven/Java packages") -- it's the non-Java multi-registry (pypi/nuget/npm/go_modules/cargo) publish watcher, deeply tied to Aspose's `data/package_registry.json` schema and shortcode content-healing, needs the same missing generic abstraction SYNC-4 needs. `baseline-ledger-writer` (S-111) is part of the KNOWLEDGE-PIPELINE (baseline SHA history for knowledge-diff), unrelated to publishing at all; not portable without target having an equivalent knowledge-baseline-tracking need, which its own scout/knowledge system doesn't currently have. | Corrected classification only, no port attempted -- see the split-out items below for individual disposition |
| SYNC-5-media-optimize | Port `media-optimize` (S-119) as a genuinely generic Hugo static-asset skill | Real, portable value found while scoping SYNC-5 -- misfiled under "Java/Maven" by name-only classification. Needs: config-driven `static/media/` path (reuse the `content_path`-template pattern from `config.yaml`), new `Pillow`+`oxipng` deps, a new backing script, and real image test fixtures. Genuinely medium effort, not attempted in this pass (already large scope this session) | Design the config extension first, then port with real tests against a small synthetic image fixture set (mirroring the `tests/fixtures/generic_hugo_repo/` pattern already established) |
...
| SYNC-10 | ✅ CLOSED 2026-08-30: read both `ht-translate.md` and `ht-translate-batch.md` in full. Decision: **do not adopt**. They wrap an entirely separate external repository (`hugo-translator`), hardcoded to an absolute machine-specific path, with its own site-profile YAML system -- a different architecture, not a generalized evolution. Target's own `translate-page`/`translate-batch` already use aspose.org's PRIOR (internal `scripts/translator/` backend) architecture and work fine. Documented in `docs/id-mapping.md`'s new "2026-08-30 decision" section | Documented decision recorded; revisit only if `hugo-translator` becomes independently relevant |

---

## Workstream GHGL-1 -- GitHub/GitLab main-branch sync automation

Source: direct assessment, 2026-08-30 -- no prior sync mechanism existed
between the `github` and `origin` (GitLab) remotes; live drift was found and
confirmed by `git rev-list` inspection, not assumed. Full design and review
record: `docs/adr/ADR-007-github-gitlab-sync.md`. Taskcards:
`reports/plan_state/GHGL-1/taskcards.jsonl`. Evidence:
`reports/plan_state/GHGL-1/evidence/`.

| ID | Task | Status | Notes |
|----|------|--------|-------|
| GHGL-1-T1 | Governance clearance: override tokens + skill context | ✅ IMPLEMENTED | Also fixed a real pre-existing tool bug hit along the way: `override_manager.py validate-for-path` stops at the first path-matching pending token in glob order regardless of expiry, instead of checking all candidates -- a stale expired token for `.gitlab-ci.yml` from 2026-07-04 was shadowing the fresh valid one until manually archived |
| GHGL-1-T2 | `scripts/ci/sync_remotes.py` + `tests/test_sync_remotes.py` | ✅ IMPLEMENTED | 10/10 tests pass against local throwaway bare repos; full repo pytest suite re-run clean |
| GHGL-1-T3 | `.github/workflows/sync-to-gitlab.yml` | ✅ IMPLEMENTED | push + hourly schedule + workflow_dispatch; no `pull_request_target` |
| GHGL-1-T4 | `.gitlab-ci.yml` `sync` stage + `sync-to-github` job | ✅ IMPLEMENTED | push-triggered only; existing 10 jobs unaffected |
| GHGL-1-T5 | GitHub secret `gitlab_token` (stored as `GITLAB_TOKEN`) + branch protection on `main` | ✅ IMPLEMENTED | force-push and deletion disabled; deliberately no required-PR-review (would block the sync bot's own direct push) |
| GHGL-1-T6 | Bootstrap fast-forward catch-up (GitLab's 2 extra commits onto GitHub) | 🟡 PAUSED | A **second, independent** divergence appeared mid-run: GitHub's own `pipeline-tests.yml` release-receipt job pushed a commit GitLab lacked, at the same time GitLab still had its 2 unique commits. No longer a clean fast-forward. Per the do-no-harm design (no auto-merge, no force-push, ever), left for human reconciliation rather than resolved automatically -- confirmed the changed file sets do not overlap, so a plain merge would very likely be clean, but that decision was not made unilaterally |
| GHGL-1-T7 | `docs/adr/ADR-007-github-gitlab-sync.md`, `.env.example`, this section | ✅ IMPLEMENTED | |
| GHGL-1-T8 | GitLab CI/CD variable `github_token` | 🔴 BLOCKED_EXTERNAL | Requires GitLab `api` scope this mission's token lacks (has only `read_repository, read_api, write_repository, create_runner, manage_runner, read_user`) and GitLab exposes no lower-privilege way to write CI/CD variables -- a human adds it via Settings > CI/CD > Variables |
