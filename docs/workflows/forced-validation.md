...
content directories and defaults to no-write mode.
...
All modes respect `--no-write` (default). Any output goes to `--scratch-root`, never content directories.

---

## Safety Gates

### validate_for_fresh() in write path
...
the changed-set comparison in `decision_engine.py`).
...
```bash
# Simulate upstream SHA change
scripts/pipeline/commands/ops/refresh_harness.py \
  --product cells/java \
  --mode synthetic-input-change \
  --fingerprint upstream_repo_sha=SYNTHETIC_TEST \
  --scratch-root runs/harness-scratch/test \
  --no-write
```
...
scripts/pipeline/commands/ops/refresh_harness.py \
  --product cells/java \
  --mode force-reconcile \
  --scratch-root runs/harness-scratch/recon01 \
  --no-write
```
...
`refresh_reconciliation_enforced=false` in production flags.

Before enforced mode can be activated:
1. validate_for_fresh() gate implemented (done)
2. No-write enforced-mode dry run verified
3. Full shadow comparison reproduced and aligned
