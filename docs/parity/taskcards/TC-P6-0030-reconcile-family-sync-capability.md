# TC-P6-0030 - Reconcile family-sync Capability

## Purpose

Resolve Phase 4 parity status for `family-sync` without weakening standalone maintainability.

## Exact Scope

Inspect `family-sync` in both inventories and implement or document the target design: Normalize this skill through the external content repo adapter so it works with standalone paths and never assumes in-repo Hugo content.
...
- `skills/family-sync.md`
- `.agents/skills/family-sync/SKILL.md`

## Files/Areas Allowed To Change

- `skills/family-sync.md`
- `.agents/skills/family-sync/SKILL.md`
- `.claude/commands/family-sync.md`
- `.kilocode/skills/family-sync/SKILL.md`
...
1. Read the aspose.org and standalone records for `family-sync` from the inventories.
2. Classify each gap category for `family-sync` as true missing behavior, intentional standalone redesign, stale reference, or verification-only.
...
- `docs/parity/evidence/family-sync-parity-verification.md`
...
`family-sync` is reclassified as parity-proven, intentionally standalone-different with evidence, or blocked with a precise next action.
