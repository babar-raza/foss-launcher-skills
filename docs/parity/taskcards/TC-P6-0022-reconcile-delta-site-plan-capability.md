# TC-P6-0022 - Reconcile delta-site-plan Capability

## Purpose

Resolve Phase 4 parity status for `delta-site-plan` without weakening standalone maintainability.

## Exact Scope

Inspect `delta-site-plan` in both inventories and implement or document the target design: Normalize this skill through the external content repo adapter so it works with standalone paths and never assumes in-repo Hugo content.
...
- `skills/delta-site-plan.md`
- `.agents/skills/delta-site-plan/SKILL.md`

## Files/Areas Allowed To Change

- `skills/delta-site-plan.md`
- `.agents/skills/delta-site-plan/SKILL.md`
- `.claude/commands/delta-site-plan.md`
- `.kilocode/skills/delta-site-plan/SKILL.md`
...
1. Read the aspose.org and standalone records for `delta-site-plan` from the inventories.
2. Classify each gap category for `delta-site-plan` as true missing behavior, intentional standalone redesign, stale reference, or verification-only.
...
- `docs/parity/evidence/delta-site-plan-parity-verification.md`
...
`delta-site-plan` is reclassified as parity-proven, intentionally standalone-different with evidence, or blocked with a precise next action.
