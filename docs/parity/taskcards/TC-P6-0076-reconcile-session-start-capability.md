# TC-P6-0076 - Reconcile session-start Capability

## Purpose

Resolve Phase 4 parity status for `session-start` without weakening standalone maintainability.

## Exact Scope

Inspect `session-start` in both inventories and implement or document the target design: Normalize this skill through the external content repo adapter so it works with standalone paths and never assumes in-repo Hugo content.
...
- `.kilocode/skills/session-start/SKILL.md`
...
1. Read the aspose.org and standalone records for `session-start` from the inventories.
2. Classify each gap category for `session-start` as true missing behavior, intentional standalone redesign, stale reference, or verification-only.
...
- `docs/parity/evidence/session-start-parity-verification.md`
...
`session-start` is reclassified as parity-proven, intentionally standalone-different with evidence, or blocked with a precise next action.
