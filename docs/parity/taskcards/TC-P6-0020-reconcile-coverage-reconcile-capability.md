# TC-P6-0020 - Reconcile coverage-reconcile Capability

## Purpose

Resolve Phase 4 parity status for `coverage-reconcile` without weakening standalone maintainability.

## Exact Scope

Inspect `coverage-reconcile` in both inventories and implement or document the target design: Triage referenced dependencies: port real skill behavior, replace site-only paths with adapters, and add wrappers for legacy references where users rely on them.
...
- `skills/coverage-reconcile.md`
- `.agents/skills/coverage-reconcile/SKILL.md`

## Files/Areas Allowed To Change

- `skills/coverage-reconcile.md`
- `.agents/skills/coverage-reconcile/SKILL.md`
- `.claude/commands/coverage-reconcile.md`
- `.kilocode/skills/coverage-reconcile/SKILL.md`
...
1. Read the aspose.org and standalone records for `coverage-reconcile` from the inventories.
2. Classify each gap category for `coverage-reconcile` as true missing behavior, intentional standalone redesign, stale reference, or verification-only.
...
- `docs/parity/evidence/coverage-reconcile-parity-verification.md`

## Risk Notes

Phase 4 status: `partial parity`. Gap categories: behavioral mismatch, missing dependency, missing helper utility. Workstreams: WS-06, WS-02, WS-04. Avoid copying website-only coupling.
...
`coverage-reconcile` is reclassified as parity-proven, intentionally standalone-different with evidence, or blocked with a precise next action.
