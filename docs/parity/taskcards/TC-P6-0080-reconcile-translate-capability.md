# TC-P6-0080 - Reconcile translate Capability

## Purpose

Resolve Phase 4 parity status for `translate` without weakening standalone maintainability.

## Exact Scope

Inspect `translate` in both inventories and implement or document the target design: Add a `/translate` compatibility dispatcher that routes to `translate-page` or `translate-batch` without duplicating translation logic.
...
1. Read the aspose.org and standalone records for `translate` from the inventories.
2. Classify each gap category for `translate` as true missing behavior, intentional standalone redesign, stale reference, or verification-only.
...
- `docs/parity/evidence/translate-parity-verification.md`

## Risk Notes

Phase 4 status: `partial parity`. Gap categories: hidden feature not surfaced cleanly, missing registration. Workstreams: WS-01. Avoid copying website-only coupling.
...
`translate` is reclassified as parity-proven, intentionally standalone-different with evidence, or blocked with a precise next action.
