# TC-P6-0006 - Reconcile batch-eval-fix Capability

## Purpose

Resolve Phase 4 parity status for `batch-eval-fix` without weakening standalone maintainability.

## Exact Scope

Inspect `batch-eval-fix` in both inventories and implement or document the target design: Normalize this skill through the external content repo adapter so it works with standalone paths and never assumes in-repo Hugo content.
...
- `.agents/skills/batch-eval-fix/SKILL.md`

## Files/Areas Allowed To Change

- `skills/batch-eval-fix.md`
- `.agents/skills/batch-eval-fix/SKILL.md`
- `.claude/commands/batch-eval-fix.md`
- `.kilocode/skills/batch-eval-fix/SKILL.md`
...
1. Read the aspose.org and standalone records for `batch-eval-fix` from the inventories.
2. Classify each gap category for `batch-eval-fix` as true missing behavior, intentional standalone redesign, stale reference, or verification-only.
...
- `docs/parity/evidence/batch-eval-fix-parity-verification.md`
...
`batch-eval-fix` is reclassified as parity-proven, intentionally standalone-different with evidence, or blocked with a precise next action.
