# TC-P6-0013 - Reconcile cleanroom-regen Capability

## Purpose

Resolve Phase 4 parity status for `cleanroom-regen` without weakening standalone maintainability.

## Exact Scope

Inspect `cleanroom-regen` in both inventories and implement or document the target design: Triage referenced dependencies: port real skill behavior, replace site-only paths with adapters, and add wrappers for legacy references where users rely on them.
...
- `.agents/skills/cleanroom-regen/SKILL.md`

## Files/Areas Allowed To Change

- `skills/cleanroom-regen.md`
- `.agents/skills/cleanroom-regen/SKILL.md`
- `.claude/commands/cleanroom-regen.md`
- `.kilocode/skills/cleanroom-regen/SKILL.md`
...
1. Read the aspose.org and standalone records for `cleanroom-regen` from the inventories.
2. Classify each gap category for `cleanroom-regen` as true missing behavior, intentional standalone redesign, stale reference, or verification-only.
...
- `docs/parity/evidence/cleanroom-regen-parity-verification.md`
...
`cleanroom-regen` is reclassified as parity-proven, intentionally standalone-different with evidence, or blocked with a precise next action.
