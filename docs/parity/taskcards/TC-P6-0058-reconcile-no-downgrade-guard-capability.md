# TC-P6-0058 - Reconcile no-downgrade-guard Capability

## Purpose

Resolve Phase 4 parity status for `no-downgrade-guard` without weakening standalone maintainability.

## Exact Scope

Inspect `no-downgrade-guard` in both inventories and implement or document the target design: Normalize this skill through the external content repo adapter so it works with standalone paths and never assumes in-repo Hugo content.
...
- `skills/no-downgrade-guard.md`
- `.agents/skills/no-downgrade-guard/SKILL.md`

## Files/Areas Allowed To Change

- `skills/no-downgrade-guard.md`
- `.agents/skills/no-downgrade-guard/SKILL.md`
- `.claude/commands/no-downgrade-guard.md`
- `.kilocode/skills/no-downgrade-guard/SKILL.md`
...
1. Read the aspose.org and standalone records for `no-downgrade-guard` from the inventories.
2. Classify each gap category for `no-downgrade-guard` as true missing behavior, intentional standalone redesign, stale reference, or verification-only.
...
- `docs/parity/evidence/no-downgrade-guard-parity-verification.md`
...
`no-downgrade-guard` is reclassified as parity-proven, intentionally standalone-different with evidence, or blocked with a precise next action.
