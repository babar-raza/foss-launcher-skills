# TC-P6-0066 - Reconcile project-phase-store Capability

## Purpose

Resolve Phase 4 parity status for `project-phase-store` without weakening standalone maintainability.

## Exact Scope

Inspect `project-phase-store` in both inventories and implement or document the target design: Normalize this skill through the external content repo adapter so it works with standalone paths and never assumes in-repo Hugo content.
...
- `skills/project-phase-store.md`
- `.agents/skills/project-phase-store/SKILL.md`

## Files/Areas Allowed To Change

- `skills/project-phase-store.md`
- `.agents/skills/project-phase-store/SKILL.md`
- `.claude/commands/project-phase-store.md`
- `.kilocode/skills/project-phase-store/SKILL.md`
...
1. Read the aspose.org and standalone records for `project-phase-store` from the inventories.
2. Classify each gap category for `project-phase-store` as true missing behavior, intentional standalone redesign, stale reference, or verification-only.
...
- `docs/parity/evidence/project-phase-store-parity-verification.md`
...
`project-phase-store` is reclassified as parity-proven, intentionally standalone-different with evidence, or blocked with a precise next action.
