# TC-P6-0047 - Reconcile locale-patch Capability

## Purpose

Resolve Phase 4 parity status for `locale-patch` without weakening standalone maintainability.

## Exact Scope

Inspect `locale-patch` in both inventories and implement or document the target design: Normalize this skill through the external content repo adapter so it works with standalone paths and never assumes in-repo Hugo content.
...
- `skills/locale-patch.md`
- `.agents/skills/locale-patch/SKILL.md`

## Files/Areas Allowed To Change

- `skills/locale-patch.md`
- `.agents/skills/locale-patch/SKILL.md`
- `.claude/commands/locale-patch.md`
- `.kilocode/skills/locale-patch/SKILL.md`
...
1. Read the aspose.org and standalone records for `locale-patch` from the inventories.
2. Classify each gap category for `locale-patch` as true missing behavior, intentional standalone redesign, stale reference, or verification-only.
...
- `docs/parity/evidence/locale-patch-parity-verification.md`
...
`locale-patch` is reclassified as parity-proven, intentionally standalone-different with evidence, or blocked with a precise next action.
