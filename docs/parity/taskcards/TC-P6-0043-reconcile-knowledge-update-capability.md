# TC-P6-0043 - Reconcile knowledge-update Capability

## Purpose

Resolve Phase 4 parity status for `knowledge-update` without weakening standalone maintainability.

## Exact Scope

Inspect `knowledge-update` in both inventories and implement or document the target design: Normalize this skill through the external content repo adapter so it works with standalone paths and never assumes in-repo Hugo content.
...
- `skills/knowledge-update.md`
- `.agents/skills/knowledge-update/SKILL.md`

## Files/Areas Allowed To Change

- `skills/knowledge-update.md`
- `.agents/skills/knowledge-update/SKILL.md`
- `.claude/commands/knowledge-update.md`
- `.kilocode/skills/knowledge-update/SKILL.md`
...
1. Read the aspose.org and standalone records for `knowledge-update` from the inventories.
2. Classify each gap category for `knowledge-update` as true missing behavior, intentional standalone redesign, stale reference, or verification-only.
...
- `docs/parity/evidence/knowledge-update-parity-verification.md`
...
`knowledge-update` is reclassified as parity-proven, intentionally standalone-different with evidence, or blocked with a precise next action.
