# TC-P6-0025 - Reconcile embed-knowledge Capability

## Purpose

Resolve Phase 4 parity status for `embed-knowledge` without weakening standalone maintainability.

## Exact Scope

Inspect `embed-knowledge` in both inventories and implement or document the target design: Normalize this skill through the external content repo adapter so it works with standalone paths and never assumes in-repo Hugo content.
...
- `.agents/skills/embed-knowledge/SKILL.md`

## Files/Areas Allowed To Change

- `skills/embed-knowledge.md`
- `.agents/skills/embed-knowledge/SKILL.md`
- `.claude/commands/embed-knowledge.md`
- `.kilocode/skills/embed-knowledge/SKILL.md`
...
1. Read the aspose.org and standalone records for `embed-knowledge` from the inventories.
2. Classify each gap category for `embed-knowledge` as true missing behavior, intentional standalone redesign, stale reference, or verification-only.
...
- `docs/parity/evidence/embed-knowledge-parity-verification.md`
...
`embed-knowledge` is reclassified as parity-proven, intentionally standalone-different with evidence, or blocked with a precise next action.
