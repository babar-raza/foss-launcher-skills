# TC-P6-0070 - Reconcile register-human-content Capability

## Purpose

Resolve Phase 4 parity status for `register-human-content` without weakening standalone maintainability.

## Exact Scope

Inspect `register-human-content` in both inventories and implement or document the target design: Normalize this skill through the external content repo adapter so it works with standalone paths and never assumes in-repo Hugo content.
...
- `skills/register-human-content.md`
- `.agents/skills/register-human-content/SKILL.md`

## Files/Areas Allowed To Change

- `skills/register-human-content.md`
- `.agents/skills/register-human-content/SKILL.md`
- `.claude/commands/register-human-content.md`
- `.kilocode/skills/register-human-content/SKILL.md`
...
1. Read the aspose.org and standalone records for `register-human-content` from the inventories.
2. Classify each gap category for `register-human-content` as true missing behavior, intentional standalone redesign, stale reference, or verification-only.
...
- `docs/parity/evidence/register-human-content-parity-verification.md`

## Risk Notes

Phase 4 status: `partial parity`. Gap categories: behavioral mismatch, missing config support, missing dependency, naming/structure mismatch. Workstreams: WS-06, WS-05, WS-02, WS-03. Avoid copying website-only coupling.
...
`register-human-content` is reclassified as parity-proven, intentionally standalone-different with evidence, or blocked with a precise next action.
