# TC-P6-0046 - Reconcile link-validate Capability

## Purpose

Resolve Phase 4 parity status for `link-validate` without weakening standalone maintainability.

## Exact Scope

Inspect `link-validate` in both inventories and implement or document the target design: Normalize this skill through the external content repo adapter so it works with standalone paths and never assumes in-repo Hugo content.
...
- `skills/link-validate.md`
- `.agents/skills/link-validate/SKILL.md`

## Files/Areas Allowed To Change

- `skills/link-validate.md`
- `.agents/skills/link-validate/SKILL.md`
- `.claude/commands/link-validate.md`
- `.kilocode/skills/link-validate/SKILL.md`
...
1. Read the aspose.org and standalone records for `link-validate` from the inventories.
2. Classify each gap category for `link-validate` as true missing behavior, intentional standalone redesign, stale reference, or verification-only.
...
- `docs/parity/evidence/link-validate-parity-verification.md`
...
`link-validate` is reclassified as parity-proven, intentionally standalone-different with evidence, or blocked with a precise next action.
