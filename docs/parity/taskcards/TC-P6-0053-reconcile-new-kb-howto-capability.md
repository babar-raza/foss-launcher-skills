# TC-P6-0053 - Reconcile new-kb-howto Capability

## Purpose

Resolve Phase 4 parity status for `new-kb-howto` without weakening standalone maintainability.

## Exact Scope

Inspect `new-kb-howto` in both inventories and implement or document the target design: Normalize this skill through the external content repo adapter so it works with standalone paths and never assumes in-repo Hugo content.
...
- `skills/new-kb-howto.md`
- `.agents/skills/new-kb-howto/SKILL.md`

## Files/Areas Allowed To Change

- `skills/new-kb-howto.md`
- `.agents/skills/new-kb-howto/SKILL.md`
- `.claude/commands/new-kb-howto.md`
- `.kilocode/skills/new-kb-howto/SKILL.md`
...
1. Read the aspose.org and standalone records for `new-kb-howto` from the inventories.
2. Classify each gap category for `new-kb-howto` as true missing behavior, intentional standalone redesign, stale reference, or verification-only.
...
- `docs/parity/evidence/new-kb-howto-parity-verification.md`
...
`new-kb-howto` is reclassified as parity-proven, intentionally standalone-different with evidence, or blocked with a precise next action.
