# TC-P6-0073 - Reconcile rubric-align Capability

## Purpose

Resolve Phase 4 parity status for `rubric-align` without weakening standalone maintainability.

## Exact Scope

Inspect `rubric-align` in both inventories and implement or document the target design: Normalize this skill through the external content repo adapter so it works with standalone paths and never assumes in-repo Hugo content.
...
- `skills/rubric-align.md`
- `.agents/skills/rubric-align/SKILL.md`

## Files/Areas Allowed To Change

- `skills/rubric-align.md`
- `.agents/skills/rubric-align/SKILL.md`
- `.claude/commands/rubric-align.md`
- `.kilocode/skills/rubric-align/SKILL.md`
...
1. Read the aspose.org and standalone records for `rubric-align` from the inventories.
2. Classify each gap category for `rubric-align` as true missing behavior, intentional standalone redesign, stale reference, or verification-only.
...
- `docs/parity/evidence/rubric-align-parity-verification.md`
...
`rubric-align` is reclassified as parity-proven, intentionally standalone-different with evidence, or blocked with a precise next action.
