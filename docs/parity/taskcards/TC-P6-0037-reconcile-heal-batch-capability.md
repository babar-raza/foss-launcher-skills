# TC-P6-0037 - Reconcile heal-batch Capability

## Purpose

Resolve Phase 4 parity status for `heal-batch` without weakening standalone maintainability.

## Exact Scope

Inspect `heal-batch` in both inventories and implement or document the target design: Normalize this skill through the external content repo adapter so it works with standalone paths and never assumes in-repo Hugo content.
...
- `skills/heal-batch.md`
- `.agents/skills/heal-batch/SKILL.md`

## Files/Areas Allowed To Change

- `skills/heal-batch.md`
- `.agents/skills/heal-batch/SKILL.md`
- `.claude/commands/heal-batch.md`
- `.kilocode/skills/heal-batch/SKILL.md`
...
1. Read the aspose.org and standalone records for `heal-batch` from the inventories.
2. Classify each gap category for `heal-batch` as true missing behavior, intentional standalone redesign, stale reference, or verification-only.
...
- `docs/parity/evidence/heal-batch-parity-verification.md`
...
`heal-batch` is reclassified as parity-proven, intentionally standalone-different with evidence, or blocked with a precise next action.
