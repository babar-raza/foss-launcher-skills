# TC-P6-0081 - Reconcile translate-batch Capability

## Purpose

Resolve Phase 4 parity status for `translate-batch` without weakening standalone maintainability.

## Exact Scope

Inspect `translate-batch` in both inventories and implement or document the target design: Normalize this skill through the external content repo adapter so it works with standalone paths and never assumes in-repo Hugo content.
...
- `.kilocode/skills/translate-batch/SKILL.md`
...
1. Read the aspose.org and standalone records for `translate-batch` from the inventories.
2. Classify each gap category for `translate-batch` as true missing behavior, intentional standalone redesign, stale reference, or verification-only.
...
- `docs/parity/evidence/translate-batch-parity-verification.md`
...
`translate-batch` is reclassified as parity-proven, intentionally standalone-different with evidence, or blocked with a precise next action.
