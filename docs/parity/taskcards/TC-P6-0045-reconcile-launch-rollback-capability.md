# TC-P6-0045 - Reconcile launch-rollback Capability

## Purpose

Resolve Phase 4 parity status for `launch-rollback` without weakening standalone maintainability.

## Exact Scope

Inspect `launch-rollback` in both inventories and implement or document the target design: Triage referenced dependencies: port real skill behavior, replace site-only paths with adapters, and add wrappers for legacy references where users rely on them.
...
- `skills/launch-rollback.md`
- `.agents/skills/launch-rollback/SKILL.md`

## Files/Areas Allowed To Change

- `skills/launch-rollback.md`
- `.agents/skills/launch-rollback/SKILL.md`
- `.claude/commands/launch-rollback.md`
- `.kilocode/skills/launch-rollback/SKILL.md`
...
1. Read the aspose.org and standalone records for `launch-rollback` from the inventories.
2. Classify each gap category for `launch-rollback` as true missing behavior, intentional standalone redesign, stale reference, or verification-only.
...
- `docs/parity/evidence/launch-rollback-parity-verification.md`

## Risk Notes

Phase 4 status: `partial parity`. Gap categories: behavioral mismatch, missing dependency. Workstreams: WS-06, WS-02. Avoid copying website-only coupling.
...
`launch-rollback` is reclassified as parity-proven, intentionally standalone-different with evidence, or blocked with a precise next action.
