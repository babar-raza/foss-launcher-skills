# TC-P6-0044 - Reconcile launch-product Capability

## Purpose

Resolve Phase 4 parity status for `launch-product` without weakening standalone maintainability.

## Exact Scope

Inspect `launch-product` in both inventories and implement or document the target design: Normalize this skill through the external content repo adapter so it works with standalone paths and never assumes in-repo Hugo content.
...
- `skills/launch-product.md`
- `.agents/skills/launch-product/SKILL.md`

## Files/Areas Allowed To Change

- `skills/launch-product.md`
- `.agents/skills/launch-product/SKILL.md`
- `.claude/commands/launch-product.md`
- `.kilocode/skills/launch-product/SKILL.md`
...
1. Read the aspose.org and standalone records for `launch-product` from the inventories.
2. Classify each gap category for `launch-product` as true missing behavior, intentional standalone redesign, stale reference, or verification-only.
...
- `docs/parity/evidence/launch-product-parity-verification.md`
...
`launch-product` is reclassified as parity-proven, intentionally standalone-different with evidence, or blocked with a precise next action.
