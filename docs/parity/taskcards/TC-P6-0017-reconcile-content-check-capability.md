# TC-P6-0017 - Reconcile content-check Capability

## Purpose

Resolve Phase 4 parity status for `content-check` without weakening standalone maintainability.

## Exact Scope

Inspect `content-check` in both inventories and implement or document the target design: Normalize this skill through the external content repo adapter so it works with standalone paths and never assumes in-repo Hugo content.
...
- `.agents/skills/content-check/SKILL.md`

## Files/Areas Allowed To Change

- `skills/content-check.md`
- `.agents/skills/content-check/SKILL.md`
- `.claude/commands/content-check.md`
- `.kilocode/skills/content-check/SKILL.md`
...
1. Read the aspose.org and standalone records for `content-check` from the inventories.
2. Classify each gap category for `content-check` as true missing behavior, intentional standalone redesign, stale reference, or verification-only.
...
- `docs/parity/evidence/content-check-parity-verification.md`
...
`content-check` is reclassified as parity-proven, intentionally standalone-different with evidence, or blocked with a precise next action.
