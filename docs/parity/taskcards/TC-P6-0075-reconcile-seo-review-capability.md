# TC-P6-0075 - Reconcile seo-review Capability

## Purpose

Resolve Phase 4 parity status for `seo-review` without weakening standalone maintainability.

## Exact Scope

Inspect `seo-review` in both inventories and implement or document the target design: Create a standalone governance-only/Claude utility skill only if SEO review is still required; keep it separate from evidence-grounded content generation and mark it non-content-writing by default.
...
1. Read the aspose.org and standalone records for `seo-review` from the inventories.
2. Classify each gap category for `seo-review` as true missing behavior, intentional standalone redesign, stale reference, or verification-only.
...
- `docs/parity/evidence/seo-review-parity-verification.md`
...
`seo-review` is reclassified as parity-proven, intentionally standalone-different with evidence, or blocked with a precise next action.
