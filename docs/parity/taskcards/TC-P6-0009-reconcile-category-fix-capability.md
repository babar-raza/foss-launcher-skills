# TC-P6-0009 - Reconcile category-fix Capability

## Purpose

Resolve Phase 4 parity status for `category-fix` without weakening standalone maintainability.

## Exact Scope

Inspect `category-fix` in both inventories and implement or document the target design: Normalize this skill through the external content repo adapter so it works with standalone paths and never assumes in-repo Hugo content.
...
- `skills/category-fix.md`
- `.agents/skills/category-fix/SKILL.md`

## Files/Areas Allowed To Change

- `skills/category-fix.md`
- `.agents/skills/category-fix/SKILL.md`
- `.claude/commands/category-fix.md`
- `.kilocode/skills/category-fix/SKILL.md`
...
1. Read the aspose.org and standalone records for `category-fix` from the inventories.
2. Classify each gap category for `category-fix` as true missing behavior, intentional standalone redesign, stale reference, or verification-only.
...
- `docs/parity/evidence/category-fix-parity-verification.md`
...
`category-fix` is reclassified as parity-proven, intentionally standalone-different with evidence, or blocked with a precise next action.
