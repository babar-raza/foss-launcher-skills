# TC-P6-0018 - Reconcile content-enrich Capability

## Purpose

Resolve Phase 4 parity status for `content-enrich` without weakening standalone maintainability.

## Exact Scope

Inspect `content-enrich` in both inventories and implement or document the target design: Create a standalone `content-enrich` skill backed by the enrichment pipeline if the feature remains current; otherwise add a documented deprecation/redirect to the cleaner evidence or gap pipeline after operator approval.
...
1. Read the aspose.org and standalone records for `content-enrich` from the inventories.
2. Classify each gap category for `content-enrich` as true missing behavior, intentional standalone redesign, stale reference, or verification-only.
...
- `docs/parity/evidence/content-enrich-parity-verification.md`

## Risk Notes

Phase 4 status: `missing entirely`. Gap categories: missing skill. Workstreams: WS-01. Avoid copying website-only coupling.
...
`content-enrich` is reclassified as parity-proven, intentionally standalone-different with evidence, or blocked with a precise next action.
