# TC-P6-0010 - Reconcile causal-backtrack Capability

## Purpose

Resolve Phase 4 parity status for `causal-backtrack` without weakening standalone maintainability.

## Exact Scope

Inspect `causal-backtrack` in both inventories and implement or document the target design: Triage referenced dependencies: port real skill behavior, replace site-only paths with adapters, and add wrappers for legacy references where users rely on them.
...
- `skills/causal-backtrack.md`
- `.agents/skills/causal-backtrack/SKILL.md`

## Files/Areas Allowed To Change

- `skills/causal-backtrack.md`
- `.agents/skills/causal-backtrack/SKILL.md`
- `.claude/commands/causal-backtrack.md`
- `.kilocode/skills/causal-backtrack/SKILL.md`
...
1. Read the aspose.org and standalone records for `causal-backtrack` from the inventories.
2. Classify each gap category for `causal-backtrack` as true missing behavior, intentional standalone redesign, stale reference, or verification-only.
...
- `docs/parity/evidence/causal-backtrack-parity-verification.md`

## Risk Notes

Phase 4 status: `partial parity`. Gap categories: behavioral mismatch, missing dependency, missing helper utility, naming/structure mismatch. Workstreams: WS-06, WS-02, WS-04, WS-03. Avoid copying website-only coupling.
...
`causal-backtrack` is reclassified as parity-proven, intentionally standalone-different with evidence, or blocked with a precise next action.
