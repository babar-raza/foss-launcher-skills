# TC-P6-0039 - Reconcile knowledge-bootstrap Capability

## Purpose

Resolve Phase 4 parity status for `knowledge-bootstrap` without weakening standalone maintainability.

## Exact Scope

Inspect `knowledge-bootstrap` in both inventories and implement or document the target design: Normalize this skill through the external content repo adapter so it works with standalone paths and never assumes in-repo Hugo content.
...
- `skills/knowledge-bootstrap.md`
- `.agents/skills/knowledge-bootstrap/SKILL.md`

## Files/Areas Allowed To Change

- `skills/knowledge-bootstrap.md`
- `.agents/skills/knowledge-bootstrap/SKILL.md`
- `.claude/commands/knowledge-bootstrap.md`
- `.kilocode/skills/knowledge-bootstrap/SKILL.md`
...
1. Read the aspose.org and standalone records for `knowledge-bootstrap` from the inventories.
2. Classify each gap category for `knowledge-bootstrap` as true missing behavior, intentional standalone redesign, stale reference, or verification-only.
...
- `docs/parity/evidence/knowledge-bootstrap-parity-verification.md`

## Risk Notes

Phase 4 status: `partial parity`. Gap categories: missing config support, missing dependency. Workstreams: WS-05, WS-02. Avoid copying website-only coupling.
...
`knowledge-bootstrap` is reclassified as parity-proven, intentionally standalone-different with evidence, or blocked with a precise next action.
