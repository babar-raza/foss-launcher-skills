# TC-P6-0077 - Reconcile site-plan Capability

## Purpose

Resolve Phase 4 parity status for `site-plan` without weakening standalone maintainability.

## Exact Scope

Inspect `site-plan` in both inventories and implement or document the target design: Normalize this skill through the external content repo adapter so it works with standalone paths and never assumes in-repo Hugo content.
...
- `skills/site-plan.md`
- `.agents/skills/site-plan/SKILL.md`

## Files/Areas Allowed To Change

- `skills/site-plan.md`
- `.agents/skills/site-plan/SKILL.md`
- `.claude/commands/site-plan.md`
- `.kilocode/skills/site-plan/SKILL.md`
...
1. Read the aspose.org and standalone records for `site-plan` from the inventories.
2. Classify each gap category for `site-plan` as true missing behavior, intentional standalone redesign, stale reference, or verification-only.
...
- `docs/parity/evidence/site-plan-parity-verification.md`
...
`site-plan` is reclassified as parity-proven, intentionally standalone-different with evidence, or blocked with a precise next action.
