# TC-P6-0019 - Reconcile content-eval Capability

## Purpose

Resolve Phase 4 parity status for `content-eval` without weakening standalone maintainability.

## Exact Scope

Inspect `content-eval` in both inventories and implement or document the target design: Normalize this skill through the external content repo adapter so it works with standalone paths and never assumes in-repo Hugo content.
...
- `skills/content-eval.md`
- `.agents/skills/content-eval/SKILL.md`

## Files/Areas Allowed To Change

- `skills/content-eval.md`
- `.agents/skills/content-eval/SKILL.md`
- `.claude/commands/content-eval.md`
- `.kilocode/skills/content-eval/SKILL.md`
...
1. Read the aspose.org and standalone records for `content-eval` from the inventories.
2. Classify each gap category for `content-eval` as true missing behavior, intentional standalone redesign, stale reference, or verification-only.
...
- `docs/parity/evidence/content-eval-parity-verification.md`
...
`content-eval` is reclassified as parity-proven, intentionally standalone-different with evidence, or blocked with a precise next action.
