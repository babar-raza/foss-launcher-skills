# TC-P6-0056 - Reconcile new-reference-index Capability

## Purpose

Resolve Phase 4 parity status for `new-reference-index` without weakening standalone maintainability.

## Exact Scope

Inspect `new-reference-index` in both inventories and implement or document the target design: Normalize this skill through the external content repo adapter so it works with standalone paths and never assumes in-repo Hugo content.
...
- `skills/new-reference-index.md`
- `.agents/skills/new-reference-index/SKILL.md`

## Files/Areas Allowed To Change

- `skills/new-reference-index.md`
- `.agents/skills/new-reference-index/SKILL.md`
- `.claude/commands/new-reference-index.md`
- `.kilocode/skills/new-reference-index/SKILL.md`
...
1. Read the aspose.org and standalone records for `new-reference-index` from the inventories.
2. Classify each gap category for `new-reference-index` as true missing behavior, intentional standalone redesign, stale reference, or verification-only.
...
- `docs/parity/evidence/new-reference-index-parity-verification.md`
...
`new-reference-index` is reclassified as parity-proven, intentionally standalone-different with evidence, or blocked with a precise next action.
