# TC-P6-0016 - Reconcile content-audit Capability

## Purpose

Resolve Phase 4 parity status for `content-audit` without weakening standalone maintainability.

## Exact Scope

Inspect `content-audit` in both inventories and implement or document the target design: Normalize this skill through the external content repo adapter so it works with standalone paths and never assumes in-repo Hugo content.
...
- `.kilocode/skills/content-audit/SKILL.md`
...
1. Read the aspose.org and standalone records for `content-audit` from the inventories.
2. Classify each gap category for `content-audit` as true missing behavior, intentional standalone redesign, stale reference, or verification-only.
...
- `docs/parity/evidence/content-audit-parity-verification.md`
...
`content-audit` is reclassified as parity-proven, intentionally standalone-different with evidence, or blocked with a precise next action.
