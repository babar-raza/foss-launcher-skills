...
**Scope of what is now anchored (updated same day, final):** 90 of 92 capabilities/artifacts now have a `source-anchors.yaml` entry — 13 fully verified with real tests (the artifacts this sync actually ported/tested: session_identity.py, 2 CI checks, the planning doc, 5 llms-* skills, workflow-harden, advisory_lock.py, taskcard_store.py, schema_validators.py), 77 mechanically anchored by `scripts/pipeline/commands/ops/backfill_source_anchors.py` (source file confirmed to exist, pinned to its own current commit SHA — explicitly **not** a semantic content re-review of those 77; see TASK_BACKLOG.md SYNC-8, which stays open until that review happens). 2 (`translate-page`/`translate-batch`) could not be anchored — source has retired them from its canonical trees (SYNC-10). Building the backfill tool also caught and fixed an independent, real documentation bug: `docs/id-mapping.md` had mislabeled 4 foss-exclusive skills as having an aspose.org counterpart.

New capabilities added beyond the original closure: `llms-generate`/`llms-coverage`/`llms-fidelity`/`llms-verify`/`llms-stale` (S-116–120), `workflow-harden` (S-121) — bringing this repo to 104 registered skills (`skills/registry.yaml`), plus the concurrency-safety core (`advisory_lock.py`, `taskcard_store.py`) which isn't itself a skill.
