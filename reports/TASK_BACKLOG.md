...
| TC-023 | Hook behavioral test (install + violating commit) | ✅ DONE | Non-blocking | themes/ write blocked (exit 1); clean commit passes (exit 0); hook strengthened in ff84fd7 |
| TC-024 | Live content repo smoke test (one skill chain) | SKIPPED | Non-blocking | No CONTENT_REPO_PATH configured; requires external content repo |
