...
id: S-97
description: >
  Layer 2 body-prose staleness scanner. Detects whether a content page's prose
  has become stale relative to the current knowledge model by scanning for
  references to changed or removed API classes and claims. Read-only.
args: "{family} {platform} [--files {glob}] [--output {path}]"
---
