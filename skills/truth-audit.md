...
id: S-90
description: >
  Verify every API claim in content files against api_surface.json at the member level —
  methods, properties, constructor signatures, enum values. Catches fabricated APIs that
  pass class-level checks.
args: "{family} {platform}"
---

# S-90: Truth Audit — Member-Level API Verification

**Arguments**: $ARGUMENTS
Expected format: `{family} {platform}` — e.g. `3d java` or `cells python`

## Purpose

Verify every API claim in content files against `api_surface.json` at the **member level** — not
just class existence but individual methods, properties, constructor signatures, enum values, and
types. This catches fabricated APIs that pass class-level checks (S-33, S-23 ground-check).

## Quick Reference — Verdict Summary

| Verdict | Trigger | Action Required |
|---------|---------|-----------------|
| **FAIL** | Method/property/class not in `api_surface.json`, wrong type, wrong install command, contradicted format claim, forbidden claim | Must fix before commit |
| **WARN** | Unverifiable member (empty methods array), not-implemented feature used without caveat, inherited member on child class | Flag for review; advisory only |
| **PASS** | Member confirmed in `api_surface.json` or parent class via `class_graph`; format claim matches `formats.md` | No action |

## Pre-conditions

1. `knowledge/{family}/{platform}/merged/api_surface.json` must exist
2. `knowledge/{family}/{platform}/merged/index.json` must exist
3. Knowledge must not be stale (`stale_since: null` in `model.yaml`)
...
5. **`limitations.md`** — methods that throw NotImplementedError or are not implemented
...
When `surface_tier == 3` and `{platform}` is `cpp`, print this explicit advisory:
```
⚠ WARNING: C++ empty-body stubs are NOT reliably detected by the API surface
  scanner unless stub detection has been applied and /repo-scout has been re-run.
  The limitations.md for this product may be INCOMPLETE.
  Manual review of the FOSS repo source is required before publishing any C++ content:
    - Check the repo include/ and src/ for functions returning false/nullptr
      unconditionally or with empty bodies {}.
    - Re-run /repo-scout {family} cpp to refresh limitations.md if not done.
  Do NOT publish C++ content until this advisory is resolved.
```

## Content Files to Scan

Glob all content files for the product using site paths from `config.yaml sites` section:
- All content files under `{site-path}/{family}/{platform}/` for each configured site

Read `config.yaml` to determine which site sections exist and their content paths.
Apply to all relevant sections: docs, blog, reference, kb, products (as configured).
...
3. If found on parent → PASS (but note inheritance in the report)
4. If not found on parent either → continue up the chain (repeat steps 1–3 for the parent's parent)
5. If not found anywhere in hierarchy → FAIL

**Deep inheritance (>2 levels)**: Walk the full chain up to a maximum of 5 levels. If the
`class_graph` entry for a class is empty (`[]`) or the class does not appear in `class_graph`,
stop traversal at that point. Do NOT assume a member exists simply because the traversal
was incomplete — treat unresolvable deep chains as WARN (UNVERIFIABLE), not FAIL, since the
api_surface extraction may be incomplete for deeply nested hierarchies.

**Circular inheritance**: If the same class appears twice during traversal (circular reference
in `class_graph`), stop immediately and emit WARN: "Circular inheritance detected for
{ClassName} — manual review required."
...
- **Line {N}**: `ClassName.method()` — exists but throws NotImplementedError per limitations.md
...
|-------|-------|----------------|
| **S-90 truth-audit** | Member-level | Fabricated methods, properties, wrong types, wrong signatures |
...
| S-23 ground-check | Structure-level | Frontmatter, file paths, class-name existence |

**Recommended chain**: Generate → **S-90** → fix FAILs → S-32 → S-33 → write

## Re-run Policy

This skill is designed to be re-run. After fixing FAILs, re-run to confirm all fixes and check
for regressions. A clean run (0 FAILs) is the gate for commit readiness.
